...
restart_comet(MyName) ->
    ...
    ?PRINT(["REstarting COMET ", wf:state('MyPid'), "  ", MyPid]),
    local_chat("System","test","Restarting your comet " ++ io_lib:format("~p", [MyPid])),
...
event(submit) ->
    ?PRINT([wf:state(username), wf:html_encode(wf:q(msg)), wf:state('MyPid')]),
...
check_global_connection(PID) ->
	...
	local_chat("Error","Error","Trying to reconnect"),
	wf:remove(".userlist"),
	wf:flush()
...
%	    ?PRINT(["Exit message from ",From , aync_die, PP, " ", self()]),
    ...
    after 10000 -> true
	%%?PRINT(wf:path_info()),
%	wf:update(placeholder2,"Ping from " ++ erlang:atom_to_list(node())),
